        ...
        this.nodeService.getFiles().then((treeFiles) => {
            ...
            this.service.getFill(id).subscribe((data) => {
                if (Array.isArray(data)) {
                    ...
                    console.error(
                        'El formato de datos devuelto no es un arreglo.'
                    );
                }
            });
        });
    ...
    findNodesByKey(
        ...
        nodes.forEach((node) => {
            ...
            if (node.children && node.children.length > 0) {
                childrenSelected = this.findNodesByKey(
                    node.children,
                    keys,
                    node
                );
            ...
            const allChildrenSelected =
                ...
                node.children.every((child) => keys.includes(child.key));
            ...
            if (parent) {
                ...
                parent.partialSelected =
                    parent.partialSelected || hasSelectedChildren;
        ...
        if (parent === null) {
            const expectedCount = nodes.reduce(
                (count, node) =>
                    count + 1 + (node.children ? node.children.length : 0),
                0
            );
